...
function printMenuManager(err, result){
  if (result.selection === '1') {
   ...
   printArrayOrderedByTitle();
   startMenu();
  } else if (result.selection === '3'){
    printArrayOrderedByPrice();
    startMenu();
...
function printArrayOrderedByTitle(){
...
function printArrayOrderedByPrice(){
  const copy = [...publicationArray];
...
function insertMagazineManger(err, result){
  ...
  const magazine = new model.Magazine(result.title, result.publisher, result.release, result.periodicy, result.type, parseFloat(result.price), parseInt(result.copies), parseFloat(result.discount));
